...
nvfp4_linear_swiglu_w4a4_tma_kernel(const __grid_constant__ CUtensorMap a_codes,
                                    const __grid_constant__ CUtensorMap b_codes,
                                    const __grid_constant__ CUtensorMap a_scales,
                                    const __grid_constant__ CUtensorMap b_scales, float alpha,
...
#pragma unroll 1
            for (int k_tile = 0; k_tile < kKTiles; ++k_tile) {
                ...
                nvfp4_tma_load_2d(tensors.b_codes[stage] + kPairN * Schedule::kCodeRowBytes,
                                  &b_codes, k_tile * Schedule::kCodeRowBytes,
                ...
                const int up_scale_row =
                    (((pair_begin + kIntermediate) / 128) * Geometry::kScaleTilesPerRow +
                     k_tile * Schedule::kK64PerStage) *
                    32;
                nvfp4_tma_load_2d(tensors.b_scales[stage][0], &b_scales, 0,
                                  gate_scale_row, &shared.full[stage]);
                nvfp4_tma_load_2d(tensors.b_scales[stage][1], &b_scales, 0,
